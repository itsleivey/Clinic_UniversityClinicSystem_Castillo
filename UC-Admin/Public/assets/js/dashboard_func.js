    ...
    if (departmentElement) {
        departmentElement.addEventListener('change', function () {
            ...
            rows.forEach(row => {
                ...
                row.style.display = selectedDepartment === "" || departmentValue === selectedDepartment
                    ? ''
                    : 'none';
            });
        });
...
function showProfileContent(clientID) {
    ...
    document.getElementById('profileDetails').innerHTML = `Displaying profile for client ID: ${clientID}`;
}
